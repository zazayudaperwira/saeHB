#' @title Small Area Estimation using Hierarchical Bayesian under Exponential Distribution
#' @description This function is implemented to variable of interest \eqn{(y)} that assumed to be a Exponential Distribution. The range of data is \eqn{ y > 0)}
...
Exponential <- function(formula,iter.update=3, iter.mcmc=10000, coef, var.coef, thin = 2, burn.in =2000, tau.u = 1, data){
  ...
  if (!any(is.na(formuladata[,1]))){
    ...
    mu.b = mu.b.value
    tau.b = tau.b.value
    tau.ua = tau.ub = a.var=phi.a=phi.b = 1
    for (i in 1:iter.update){
      ...
      jags.m <- jags.model( file = "HBExp.txt", data=dat, inits=inits, n.chains=1, n.adapt=500 )
      file.remove("HBExp.txt")
    ...
    for (i in 1:iter.update){
      ...
      cat("model {
					...
					for(j in 1:n2) {
					  	...
					  	y_nonsampled[j] ~ dexp(lambda.nonsampled[j])
					  	lambda.nonsampled[j]<-1/mu.nonsampled[j]

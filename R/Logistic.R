#' @title Small Area Estimation using Hierarchical Bayesian under Logistic Distribution
#' @description This function is implemented to variable of interest \eqn{(y)} that assumed to be a Logistic Distribution
...
Logistic <- function(formula,iter.update=3, iter.mcmc=10000, coef, var.coef, thin = 2, burn.in =2000, tau.u = 1, data){
  ...
  if (!any(is.na(formuladata[,1]))){
    ...
    for (i in 1:iter.update){
      ...
      jags.m <- jags.model(file = "Logistic.txt", data=dat, inits=inits, n.chains=1, n.adapt=500  )
      file.remove("Logistic.txt")
    ...
    for (i in 1:iter.update){
      ...
      cat("model {
          ...
          for (j in 1:n2) {
          	  v[j]~dnorm(0,tau.u)
					  	y_nonsampled[j] ~ dlogis(mu.nonsampled[j],tau.nonsampled[j])

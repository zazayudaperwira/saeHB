#' @title Small Area Estimation using Hierarchical Bayesian under Poisson Gamma Distribution
#' @description This function is implemented to variable of interest \eqn{(y)} that assumed to be a Poisson Distribution which it is parameter \eqn{(\lambda)} is assumed to be a Gamma distribution. The data is a count data, \eqn{y = 1,2,3,...}
...
PoissonGamma <- function(formula,iter.update=3, iter.mcmc=10000, coef, var.coef, thin = 2, burn.in =2000, tau.u = 1, data){
  ...
  if (!any(is.na(formuladata[,1]))){
    ...
    dif <- round(formuladata[,1])-formuladata[,1]
    if (any(abs(dif) > 0) &  any(abs(dif) < 1)){
      stop("response variable must be count data, " ,formula[2], " = 0, 1, 2, ...")
    }
    ...
    for (i in 1:iter.update){
      ...
      cat("model {
					for (i in 1:n) {
							y[i] ~ dpois(mu[i])
							mu[i]~ dgamma(lambda.a[i],lambda.b[i])
							lambda.a[i] = pow(lambda[i],2)*tau.u
							lambda.b[i] = lambda[i]*tau.u
							log(lambda[i]) <- b[1] + sum(b[2:nvar]*x[i,])
          ...
          a.var <- 1/tau.u
		  	...
		  	}", file="PoissonGamma.txt")
      jags.m <- jags.model( file = "PoissonGamma.txt", data=dat, inits=inits, n.chains=1, n.adapt=500 )
      file.remove("PoissonGamma.txt")
    ...
    dif <- round(data_sampled[,1])-data_sampled[,1]
    if (any(abs(dif) > 0) &  any(abs(dif) < 1)){
      stop("response variable must be count data, " ,formula[2], " = 0, 1, 2, ...")
    }
    ...
    for (i in 1:iter.update){
      ...
      cat("model {
          for (i in 1:n1) {
							y_sampled[i] ~ dpois(mu[i])
							mu[i]~ dgamma(lambda.a[i],lambda.b[i])
							lambda.a[i] = pow(lambda[i],2)*tau.u
							lambda.b[i] = lambda[i]*tau.u
							log(lambda[i]) <- b[1] + sum(b[2:nvar]*x_sampled[i,])
          }

          for(j in 1:n2) {
              mu.nonsampled[j]~ dgamma(lambda.a.nonsampled[j],lambda.b.nonsampled[j])
							lambda.a.nonsampled[j] = pow(lambda.nonsampled[j],2)/a.var
							lambda.b.nonsampled[j] = lambda.nonsampled[j]/a.var
							log(lambda.nonsampled[j])=  mu.b[1] + sum(mu.b[2:nvar]*x_nonsampled[j,])
              }

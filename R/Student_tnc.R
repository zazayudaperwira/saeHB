#' @title Small Area Estimation using Hierarchical Bayesian under Student-t (non central) Distribution
#' @description This function is implemented to variable of interest \eqn{(y)} that assumed to be a Student-t (non central) Distribution. The range of data is \eqn{(-\infty < y < \infty)}
...
#'
#' }
Student_tnc <- function(formula,iter.update=3, iter.mcmc=10000,coef, var.coef, thin = 2, burn.in =2000, tau.u = 1, data){
    ...
    for (i in 1:iter.update){
      ...
      cat("model {
          for (i in 1:n1) {
          	  ...
          	  y_sampled[i] ~ dnt(mu[i],tau[i],phi[i])
          ...
          for (j in 1:n2) {
          		v[j] ~ dnorm(0, tau.u)
					    y_nonsampled[j] ~ dnt(mu.nonsampled[j],tau.nonsampled[j],phi.nonsampled[j])

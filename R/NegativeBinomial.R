#' @title Small Area Estimation using Hierarchical Bayesian under Negative Binomial Distribution
#' @description This function is implemented to variable of interest \eqn{(y)} that assumed to be a Negative Binomial Distribution. The data is a number of the Bernoulli process. The negative binomial is used to overcome an over dispersion from the discrete model.
  ...
  if (!any(is.na(formuladata[,1]))){
    ...
    for (i in 1:iter.update){
      ...
      cat("model {
          for (i in 1:n) {
          		y[i] ~ dnegbin(p[i],r[i])
							p[i]<-mu[i]*phi[i]/(1+mu[i]*phi[i])
							r[i]<-pow(mu[i],2)*phi[i]
      ...
      jags.m <- jags.model(file = "Negbin.txt", data=dat, inits=inits, n.chains=1, n.adapt=500  )
      file.remove("Negbin.txt")
    ...
    for (i in 1:iter.update){
      ...
      cat("model {
          ...
          for (j in 1:n2) {
          		y_nonsampled[j] ~ dnegbin(p.nonsampled[j],r.nonsampled[j])
							p.nonsampled[j]<-mu.nonsampled[j]*phi.nonsampled[j]/(1+mu.nonsampled[j]*phi.nonsampled[j])
							r.nonsampled[j]<-pow(mu.nonsampled[j],2)*phi.nonsampled[j]
